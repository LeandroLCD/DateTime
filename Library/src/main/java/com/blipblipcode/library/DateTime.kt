...
class DateTime private constructor(
    ...
    ):this(zoneDateTime.year, zoneDateTime.monthValue, zoneDateTime.dayOfMonth, zoneDateTime.hour, zoneDateTime.minute, zoneDateTime.second, zoneDateTime.zone.id)
    init {
    ...
    companion object {
        ...
        fun fromString(dateString: String): DateTime {
            val normalizedDate = dateString.replace(Regex("[/.-]"), "-")
            val patterns = listOf(
                "dd-MM-yyyy HH:mm:ss", "dd-MM-yyyy HH:mm", "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm",
                "dd-MM-yyyy", "yyyy-MM-dd", "d-M-yyyy", "yyyy-M-d"
            )

            val parsedDateTime = patterns.firstNotNullOfOrNull { pattern ->
                runCatching {
                    LocalDateTime.parse(normalizedDate, DateTimeFormatter.ofPattern(pattern))
                }.getOrNull()
    ...
    fun addDays(days: Long): DateTime {
        val updatedDate = LocalDateTime.of(year, month, day, hour, minute, second).plusDays(days)
        return copy(updatedDate)
    }

    fun addYears(years: Long): DateTime {
        val updatedDate = LocalDateTime.of(year, month, day, hour, minute, second).plusYears(years)
        return copy(updatedDate)
    }

    fun addMinutes(minutes: Long): DateTime {
        val updatedDate = LocalDateTime.of(year, month, day, hour, minute, second).plusMinutes(minutes)
        return copy(updatedDate)
    }

    fun addSeconds(seconds: Long): DateTime {
        val updatedDate = LocalDateTime.of(year, month, day, hour, minute, second).plusSeconds(seconds)
        return copy(updatedDate)
    }

    fun timeSpan(other: DateTime): TimeSpan {
        val thisDateTime = LocalDateTime.of(year, month, day, hour, minute, second)
        val otherDateTime = LocalDateTime.of(other.year, other.month, other.day, other.hour, other.minute, other.second)
        ...
        return TimeSpan(years.toInt(), months.toInt(), days.toInt(), hours.toInt(), minutes.toInt(), seconds.toInt())
    }
